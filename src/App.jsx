...
import { AlertTriangle, Clock, User, Calendar, MapPin, Stethoscope, CheckCircle } from 'lucide-react';
...
const EWSClickForm = () => {
  ...
  const ewsParameters = {
    respiratoryRate: {
      ...
      options: [
        ...
        { label: '9-11 /menit', value: '9-11', score: 1, color: 'bg-yellow-500' },
        { label: '12-20 /menit (Normal)', value: '12-20', score: 0, color: 'bg-green-500' },
        { label: '21-24 /menit', value: '21-24', score: 2, color: 'bg-orange-500' },
    ...
    spo2: {
      ...
      options: [
        ...
        { label: '> 96% (Normal)', value: '>96', score: 0, color: 'bg-green-500' }
      ]
    },
    oxygenSupport: {
    ...
    systolicBP: {
      ...
      options: [
        ...
        { label: '41-50 mmHg', value: '41-50', score: 1, color: 'bg-yellow-500' },
        { label: '51-60 mmHg', value: '51-60', score: 0, color: 'bg-green-500' },
        { label: '61-70 mmHg', value: '61-70', score: 0, color: 'bg-green-500' },
        { label: '71-80 mmHg', value: '71-80', score: 0, color: 'bg-green-500' },
        { label: '81-90 mmHg', value: '81-90', score: 0, color: 'bg-green-500' },
        { label: '91-100 mmHg', value: '91-100', score: 1, color: 'bg-yellow-500' },
        { label: '101-110 mmHg', value: '101-110', score: 1, color: 'bg-yellow-500' },
        { label: '111-120 mmHg', value: '111-120', score: 2, color: 'bg-orange-500' },
        { label: '121-130 mmHg', value: '121-130', score: 2, color: 'bg-orange-500' },
    ...
    heartRate: {
      ...
      options: [
        ...
        { label: '41-50 /menit', value: '41-50', score: 1, color: 'bg-yellow-500' },
        { label: '51-90 /menit (Normal)', value: '51-90', score: 0, color: 'bg-green-500' },
        { label: '91-110 /menit', value: '91-110', score: 1, color: 'bg-yellow-500' },
        { label: '111-130 /menit', value: '111-130', score: 2, color: 'bg-orange-500' },
    ...
    temperature: {
      ...
      options: [
        ...
        { label: '36-37°C (Normal)', value: '36-37', score: 0, color: 'bg-green-500' },
  ...
  const morseScaleParameters = {
    ...
    statusMental: {
      ...
      icon: '',
      options: [
        { label: 'Orientasi Baik', value: 'normal', score: 0, color: 'bg-green-500' },
        { label: 'Lupa Keterbatasan', value: 'disordered', score: 15, color: 'bg-orange-500' }
      ]
    },
    skalaNyeri: {
      name: 'Skala Nyeri ( 0-10 )',
      icon: '',
      options: [
  ...
  const calculateTotalScore = (vitals) => {
    ...
    Object.keys(ewsParameters).forEach(param => {
      if (vitals[param]) {
        const option = ewsParameters[param].options.find(opt => opt.value === vitals[param]);
  ...
  const calculateMorseScore = (morseVitals) => {
    ...
    Object.keys(morseScaleParameters).forEach(param => {
      if (morseVitals[param]) {
        const option = morseScaleParameters[param].options.find(opt => opt.value === morseVitals[param]);
        if (option) totalScore += option.score;
      }
    });
    return totalScore;
  ...
  const getRiskCategory = (score) => {
    if (score >= 7) return { category: 'TINGGI', color: 'bg-red-500', textColor: 'text-red-700', bgLight: 'bg-red-100' };
    else if (score >= 4) return { category: 'SEDANG', color: 'bg-yellow-500', textColor: 'text-yellow-700', bgLight: 'bg-yellow-100' };
    else if (score >= 2) return { category: 'RENDAH', color: 'bg-green-400', textColor: 'text-green-700', bgLight: 'bg-green-100' };
    else return { category: 'NORMAL', color: 'bg-green-500', textColor: 'text-green-700', bgLight: 'bg-green-100' };
  };

  const getScaleRiskCategory = (score) => {
    if (score <= 24) return { category: 'RENDAH', color: 'bg-green-500', textColor: 'text-green-700', bgLight: 'bg-green-100' };
    else if (score > 24 && score <= 44) return { category: 'SEDANG', color: 'bg-yellow-500', textColor: 'text-yellow-700', bgLight: 'bg-yellow-100' };
    else if (score >= 45) return { category: 'TINGGI', color: 'bg-red-400', textColor: 'text-red-700', bgLight: 'bg-red-100' };
  };  
  ...
  const allData = {
    ...
    try {
      await fetch('https://script.google.com/macros/s/AKfycbyKMvBYvHehzUZQQM7ID8QapUDLqWcEgPDpLFlDtW0qgkPe8m9dpW_eKQUiNzueiyVR/exec', {
        method: 'POST',
        mode: 'no-cors',
        credentials: 'include', 
        headers: {
  ...
  const getHourScore = (hour) => {
    ...
    return vitals ? calculateTotalScore(vitals) : 0;
  };
    ...
    <div className="h-screen w-full bg-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setPatientData(prev => ({...prev, medicalRecord: e.target.value}))}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Kamar Rawat</label>
            <input
              type="text"
              value={patientData.ward}
              onChange={(e) => setPatientData(prev => ({...prev, ward: e.target.value}))}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className={`${riskInfo.color} text-white p-4 rounded-lg text-center shadow-lg`}>
        ...
        {/* Tindakan */}
        <div className={`${riskInfo.bgLight} p-4 rounded-lg shadow border-l-4 ${riskInfo.color.replace('bg-', 'border-')}`}>
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        ...
        <div className="space-y-6">
          {Object.entries(ewsParameters).map(([paramKey, param]) => (
            <div key={paramKey} className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {param.options.map((option, index) => (
                  <button
                    key={index}
                    onClick={() => updateParameter(selectedHour, paramKey, option.value)}
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className={`text-xs mt-1 ${
                          currentVitals[paramKey] === option.value ? 'text-white' : 'text-gray-500'
                        }`}>
                          Skor: {option.score}
                        </div>
                      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {hours.map(hour => {
                ...
                return (
                  <tr key={hour} className={`border-t hover:bg-gray-50 ${isRequired ? 'bg-blue-50' : ''}`}>
                    <td className="p-3 font-medium">
                      {hour.toString().padStart(2, '0')}:00
                      {isRequired && <span className="ml-1 text-blue-600 font-bold">●</span>}
                    </td>
                    <td className="p-3 text-center text-xs">{vitals?.respiratoryRate || '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.spo2 || '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.oxygenSupport === 'yes' ? 'Ya' : vitals?.oxygenSupport === 'no' ? 'Tidak' : '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.systolicBP || '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.heartRate || '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.temperature || '-'}</td>
                    <td className="p-3 text-center text-xs">{vitals?.consciousness === 'alert' ? 'Alert' : vitals?.consciousness === 'vpu' ? 'V/P/U' : '-'}</td>
        ...
        <div className="mt-4 text-sm text-gray-600">
          <p><span className="text-blue-600 font-bold">●</span> = Jam wajib pemantauan</p>
        </div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 p-4 rounded-lg">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          ...
          <li>• Klik tombol yang sesuai dengan kondisi pasien untuk setiap parameter</li>
          <li>• Skor akan dihitung otomatis dan status risiko akan muncul</li>
          <li>• Sistem akan memberikan panduan tindakan yang harus dilakukan</li>
          <li>• Jam-jam wajib pemantauan akan ditandai dengan titik biru (●)</li>
        </ul>
      </div>
    </div>
  );
